  ...
  useEffect(() => {
    const loadTenancies = async () => {
      ...
      try {
        ...
        const data = await getAgentTenancies(user.uid);

        setTenancies(Array.isArray(data) ? data : []);
      } catch (err) {
        console.error("Failed to load agent tenancies:", err);
        ...
        <div className="mx-auto max-w-7xl">
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
              Manage tenants and rented properties under your management.
            </p>
          </motion.div>
            ...
            >
              {filteredTenancies.map((tenancy) => (
                <TenancyCard
                  key={tenancy.id}
                  tenancy={tenancy}
                  onView={() => navigate(`/agent/tenancies/${tenancy.id}`)}
                />
              ))}
            </motion.div>
          )}
